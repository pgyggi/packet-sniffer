...
func New(config []FilterConfig) (*FilterList, error) {
	...
	for _, filterConfig := range config {
		...
		if filterConfig.AddFields != nil {
			Filters.Register(NewAddFields(filterConfig.AddFields.Fields, filterConfig.AddFields.Conditions))
		}
		...
		if filterConfig.ConvertFields != nil {
			Filters.Register(NewConvertFields(filterConfig.ConvertFields.Converts, filterConfig.ConvertFields.DeleteOrigin))
		}

		if filterConfig.ReplaceFields != nil {
			Filters.Register(NewReplaceFields(filterConfig.ReplaceFields.Replacement))
		}

		if filterConfig.GrokFields != nil {
			Filters.Register(NewGrokFields(filterConfig.GrokFields.Patterns, filterConfig.GrokFields.Fields, filterConfig.GrokFields.Splite, filterConfig.GrokFields.Pattern, filterConfig.GrokFields.DelOrigin))
		}

		if filterConfig.IPFields != nil {
			Filters.Register(NewIPFields(filterConfig.IPFields.Fields))
		}
