...
func NewFlows(pub publish.Flows, settings *settings.Flows) (*Flows, error) {
	...
	timeout, err := duration(settings.Timeout, defaultTimeout)
	if err != nil {
		logp.Err("failed to parse flow timeout: %v", err)
		return nil, err
	}

	period, err := duration(settings.Period, defaultPeriod)
	if err != nil {
