...
type SnifferSetup struct {
	...
	config         *settings.InterfacesConfig
	isAlive        bool
	dumper         *pcap.Dumper
...
func (sniffer *SnifferSetup) setFromConfig(config *settings.InterfacesConfig) error {
	var err error
...
func (sniffer *SnifferSetup) Init(test_mode bool, factory WorkerFactory) error {
	...
	if !test_mode {
		err = sniffer.setFromConfig(&settings.SettingSingleton.Interfaces)
		if err != nil {
