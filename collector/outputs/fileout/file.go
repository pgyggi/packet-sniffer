...
func New(cfg *common.Config, _ int) (outputs.Outputer, error) {
	...
	output := &fileOutput{beatName: config.Filename}
...
func (out *fileOutput) PublishEvent(s outputs.Signaler, opts outputs.Options, event common.MapStr) error {
	...
	if err != nil {
		if opts.Guaranteed {
			...
			//fmt.Println("XXXXXX失败--2:",err)
		} else {
			logp.Err("Error when writing line to file: %s", err)
			//fmt.Println("XXXXXX失败--3:",err)
		}
		//暂不做处理不影响运行
		err = nil
	}
