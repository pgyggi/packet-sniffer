...
func LoadByte(content []byte) (*common.Config, error) {
	content = expandEnv(content)
	config, err := common.NewConfigWithYAML(content, "")
	if err != nil {
		return nil, fmt.Errorf("YAML config parsing failed : %v", err)
	}
	return config, nil
}
...
func Load(path string) (*common.Config, error) {
	if path == "" {
		path = "/etc/packet-sniffer/config.yml"
	}
